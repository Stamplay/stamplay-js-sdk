suite('Webhook', function () {
  ...
  test('has the url property', function () {
    ...
    assert.equal(webhook.url, '/api/webhook/' + Stamplay.VERSION + '/mywebhook/catch');
  });

  test('that the url is formatted correctly', function () {
    var w1 = new Stamplay.Webhook('Test_Test');
    assert.equal(w1.url, '/api/webhook/' + Stamplay.VERSION + '/test_test/catch');

    var w2 = new Stamplay.Webhook('a_b-c');
    assert.equal(w2.url, '/api/webhook/' + Stamplay.VERSION + '/a_bc/catch');
  });
